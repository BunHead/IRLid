...
const CACHE_VERSION = 'irlid-shell-v57'; // v5.11.19a — Captain's follow-up after v5.11.19 smoke: (1) Debug QR URL back to full-width across the whole frame (undoing the v5.11.19 max-width 72ch narrowing). (2) Buttons row pulled up a touch (p margin-bottom 12px → 8px, h4 margin-bottom 6px → 4px, qr-actions margin-bottom 12px → 10px). (3) Wired the genuine venue QR into the inline clone: new renderRealVenueQrIntoInlineClone() helper renders the real venueQrPayload into the clone's .v511-theme-preview-qr slot at 500x500 with QRCode.js M-level error correction; called after every observer-fired re-clone (so theme edits don't revert it to the Settings placeholder) and from generateVenueQR (so Refresh QR button updates the clone too). (4) Removed the temporary click-to-fire handler on the inline clone — real check-ins via triggerAcceptCycleAnimation → v511ActiveCelebrationStage already route effects to the inline clone; the ▶ Test anim button in .qr-info remains as the permanent debug affordance.
