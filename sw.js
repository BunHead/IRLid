...
const CACHE_VERSION = 'irlid-shell-v38'; // v5.11.2 — CSS-only fix for theme bg-image rendering in the fullscreen overlay. Added .irlid-qr-fullscreen.bg-mode-image::before pseudo-element rules (single-axis + 3 symmetry variants: horizontal, vertical, quad) mirroring the .v511-theme-preview-stage.bg-mode-image::after pattern from Org.html L5304+. Uses ::before so existing overlay children (logo, title, holder, refresh) stack above without z-index tweaks. Reads --bg-image-url / --bg-image-pos / --bg-image-size / --bg-image-repeat / --sym-pos-1..4 CSS variables already set by applyV511BackgroundToHostStages at L15948-15955. No JS timing race — pseudo renders whenever the .bg-mode-image class is present on the overlay (regardless of fullscreen state). Replaces the failed v5.11.0y JS-listener approach that triggered browser auto-exit-on-mutation.
