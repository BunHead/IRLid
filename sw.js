...
const CACHE_VERSION = 'irlid-shell-v165'; // v6.4.14 re-hire/re-invite: a removed member re-invited on their SAME already-enrolled device used to dead-end (accept screen was gated out + Worker rejected the known device). Fix: fresh invites take precedence over session-restore at boot + accept gate relaxes for a freshly-scanned invite (Org.html Client A/B) + Worker orgInviteAcceptOnThisDevice re-homes a known pub_fp via UPSERT membership. PRIOR: v163 (v6.4.13) was the ROOT-CAUSE of the check-in QR sliding off-screen after the celebration: v511EnsureRuntimeStage resolved the QR via ':scope > img' fallback, which matched the stage LOGO (a direct-child <img>) on the inline+fullscreen clones (their QR is a .v511-theme-preview-qr div, not a .qr-box). The logo got tagged .v511-theme-preview-qr, so it inherited the white QR-slot background (Captain's "logo goes white-box"), took the QR-motion transforms, AND made --qr-cx/cy compute from the logo (top of stage) so radial/pivot effects flung the QR off-screen. Fix: exclude the logo from QR resolution + strip the stray class. Verified on localhost: logo no longer tagged, bg transparent, --qr-cy tracks real QR (68.3%) not logo (2.1%). v162 was the v6.4.12 logo definite-height (kept; harmless).
