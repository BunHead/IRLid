...
const CACHE_VERSION = 'irlid-shell-v40'; // v5.11.4 — Number One's v5.11.3 comment fix removed the ORIGINAL backticks at L58 but added MORE backticks at L68 in the explanatory comment block (around the literal phrase describing the previous template literal opening). Same exact bug pattern — backticks inside the qr-fullscreen.js style.textContent template literal still terminated the literal prematurely, just at L68 instead of L58, breaking the IIFE in the same way. v5.11.4 rewrites the comment block in plain prose with zero backticks or any code-formatting delimiters. Lesson banked for BOOTSTRAP §6: never use ANY backtick character in a comment that lives inside a template literal, even when describing the very bug that backticks-in-template-literals cause.
