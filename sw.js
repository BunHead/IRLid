...
const CACHE_VERSION = 'irlid-shell-v39'; // v5.11.3 — ROOT CAUSE FIX. My v5.11.0x edit to qr-fullscreen.js added a comment with backticks around CSS property values, INSIDE the style.textContent template literal at L49. Backticks terminated the literal prematurely → IIFE syntax error → window.IRLidQrFullscreen was never registered → fullscreenQR() at Org.html L14438 saw the global as undefined and fell through to legacy Path B (which builds .qr-fullscreen-overlay, NOT .irlid-qr-fullscreen). Every fullscreen-overlay symptom since v5.11.0x deploy (no dragons, "regression after v5.11.0y", v5.11.1 not helping, v5.11.2 CSS having no effect) was downstream of THIS template literal break. Fix: replaced backticks with single quotes in the comment.
