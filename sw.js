...
const CACHE_VERSION = 'irlid-shell-v56'; // v5.11.19 — Captain's polish pass after v5.11.18 smoke. (1) Inline frame trimmed: height clamp(380px, calc(100dvh - 320px), 720px), max 68dvh, so buttons row + h4 + debug URL all sit comfortably below without push-down on smaller laptops. (2) .qr-info reordered per Captain's reference image: h4 + p left-aligned under the frame, then centred buttons row, then Debug QR URL (now narrower at max-width min(100%, 72ch) instead of full-width). (3) Caption "Live preview. Save All applies pending edits. Sample fires the selected outcome. Double-click to fullscreen." removed from source HTML — kills it from Settings preview, inline clone, and fullscreen clone in one stroke. (4) ACTIVE MODE badge stripped from inline clone only (Settings preview keeps it for the configurator UI).
