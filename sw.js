...
const CACHE_VERSION = 'irlid-shell-v47'; // v5.11.11 — broaden v5.11.10 mirror filter from narrow prefix whitelist to a CONFIG-vs-TRANSIENT blacklist for cel-*. CSS combinators like .cel-pulse.cel-int-subtle require BOTH the transient class (added by runtime per-stage) AND the configured class (set on sampleStage by Settings). v5.11.10 excluded ALL cel-* classes, so cel-int-strong/medium/subtle, cel-z-below, cel-stream-anchor-* etc. didn't reach #venueQRWrap, and celebration animations default-fired (most effects "just missing" per Captain). v5.11.11 uses explicit blacklist of transient classes (v511-runtime-stage, firing-*, cel-pulse, cel-bg, cel-qr, cel-glow, cel-pattern, cel-text, cel-outer-glow, cel-spotlight, cel-cam-flash, cel-strobe, cel-shake, cel-scale-punch, cel-iris-wipe, cel-ripple, cel-qr-disappear, cel-bounce-eff-loop, cel-confetti, cel-sparkles, cel-stream, cel-flame, cel-glitch) and mirrors EVERYTHING else cel-*/bg-*/pattern-*. Blacklist source: L7318-7320 transient class lists.
