...
const CACHE_VERSION = 'irlid-shell-v41'; // v5.11.5 — ARCHITECTURAL REWRITE per Captain's directive after hours of patching parallel implementations. fullscreenQR() in Org.html L14436 now clones #v511ThemePreviewStage (the working Settings preview stage with proven theme decoration) and uses the clone as the fullscreen target. Strips Sample-only configurator elements, updates banner to real org name, renders production venue QR, adds close button, inline-styles the clone to fill viewport, calls requestFullscreen. applyV511BackgroundToHostStages + syncBgImageMirrorLayers extended to iterate over the clone (#v511FullscreenStage). v511ActiveCelebrationStage now prefers the clone when present so real check-in celebrations fire on it. Replaces the IRLidQrFullscreen + legacy qr-fullscreen-overlay paths entirely. Every theme element that works in Settings preview works for free in Check-in fullscreen because it IS the Settings stage.
