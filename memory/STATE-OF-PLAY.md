...
**Last refreshed:** Tuesday 19 May 2026 late morning — **`v5.11` mockup celebration-animation rebuild: T4.1.10 → T4.3.2.** Long Tuesday morning watch. Captain came in Tuesday morning, fresh after a bath stretch where three spec drafts landed (`SETTINGS-REVAMP-SPEC.md`, `CALENDAR-SPEC.md`, `PROTOCOL-Records-Broker.md`). From there: Visual theming tab restructured deeply. **T4.1.10**: Pattern + Image overlays nested INSIDE Background body; QR customization nested INSIDE Post-Accept body via JS reparenting (HTML stays adjacent for source readability, DOM gets reparented at init via `mountQrInsidePostAccept()`). **T4.1.11**: Background closed by default; Image overlay above Pattern overlay; 9-position image grid shrunk to 56px cells matching cycle-direction grid; Bounce converted from checkbox to chip group; QR customization moved up inside Post-Accept body to right below Outcome sound row; "Post" sub-expander wraps remaining post-accept settings; top-level expander renamed `Post-Accept behaviour` → **`Accept behaviour`** (with `Review behaviour` / `Deny behaviour` dynamic). **T4.1.12**: Celebration palette is now PER-MODE (Captain's call) — `modeStates` carries `celPalette: [...colors]` per mode with mode-tinted defaults (Allow greens / Review ambers / Deny reds); `snapshotMode` + `loadMode` extended; `snapshotCelPalette` + `loadCelPaletteForMode` helpers. **T4.1.13**: Hotfix Sample button — dropped dead `.v511-subpanel[data-v511-subpanel="qr"]` selector left over from T4.1.9 sub-tab retirement; global `.v511-celeb-effect` query restored. **T4.2**: Biggest re-architecture since Tier 1 — Celebration animation totally rebuilt as a **library + sequence two-pane builder**: 20 effects across 5 expandable groups (Light/Motion/Particles/Surface/Timing), drag from library to sequence, each item has its own duration field, ⏱ Delay rows time gaps between effects, Sample button plays sequence top-to-bottom via async `v511PlaySequence`, per-mode sequences (Allow/Review/Deny each own a sequence array), 14 new effect CSS keyframes added (Outer glow / Spotlight / Camera flash / Strobe ⚠ / Shake / Scale punch / Iris wipe / Ripple / QR disappear / Confetti / Sparkles / Particles / Flame / Glitch); old global cycle-duration slider retired; checkbox grid retired; sequence persists per mode via existing `modeStates` capture. **T4.3**: Third frame under Library/Sequence — **Effect settings pane** with per-effect schemas (Captain's design): click a sequence row → row gets blue selected outline → settings render below with chip groups for Intensity / Bounce / Rotation / Zoom / Variant / Count / Thickness / Density and a mini 9-cell direction grid for Direction / Anchor. Settings persist into `item.settings` via the existing modeStates capture. **T4.3.1**: Sequence-playback hotfix — double `requestAnimationFrame` between effect-class remove and next add so the browser PAINTS the removal before the next mutation, otherwise CSS keyframes never restart and only the first sequence step ever played. Also renamed `data-effect="cel-pattern"` → `pattern` to stop `'cel-' + effect` from producing `cel-cel-pattern` (unmatched class); load-time migration converts old saves. **T4.3.2**: Settings → playback wiring — `v511ApplyItemSettings(stage, item)` translates `item.settings` into stage data-attrs / classes / CSS variables BEFORE the effect class is applied. Several visual fixes shipped: Text overlay bumped to `z-index: 6` (was hidden behind caption); font weight + size + shadow strengthened; full text-overlay variants wired (position top/centre/bottom, size small/medium/large, template `name` / `name + action` / `welcome`); Pattern flash moved from `::after` to `cel-fx-layer` so it stops clobbering the image overlay; 8 pattern styles wired via `data-cel-pattern-style`; Confetti changed from one-row `repeat-x` to full-area `repeat` cascade + direction setting (top/bottom/left/right via 9-cell `data-cel-dir`); Sparkles got matching direction setting; Flame anchor setting (`centre` / `image-anchor`) — pick image-anchor and the flame rises from `var(--bg-image-pos)`.

**Status at watch end:** Captain reports the mockup is "very very good" structurally but identified gaps that didn't get fixed before Captain called for rest (he had poor sleep the previous night). **Known gaps for next watch** — captured in `memory/pending-work.md` Tuesday morning section. Build pill at watch close: `v5.10.2 + v5.11 mockup T4.3.2`.
