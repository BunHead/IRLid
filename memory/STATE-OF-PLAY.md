...
**Last refreshed:** Tuesday 12 May 2026 ~22:45 demo-eve final — **`v5.9.0.13.26` LIVE; Mr. Data turbo'd both queued briefs into PRs awaiting review.** PR #13 (`v5.9.0.13.27` — symmetric background image, branch `codex/v5.9.0.13.27-bg-symmetric`, commit `355cdde`, validated via `node --check irlid-api-org/src/index.js` + `npm test`) and PR #14 (`v5.9.0.13.28` — role vocabulary carries to Viewing-as + role pills, branch `codex/v5.9.0.13.28-role-labels-carryover`, commit `ae8a203`, validated via `git diff --check` + `npm test`). Mr. Data noted he left `memory/pending-work.md` untouched (correctly identified Number One's late-night idea-bank as unrelated to either PR — good agent discipline). **Do not merge tonight** — review with fresh eyes on demo morning. Late-night idea stream banked in `pending-work.md` (logo wobble / GIF import / WAV on accept / particle-dissolve QR / glow trail / layer system). Captain's local main also has Number One's multi-fp Worker patch + pill bump + brief files + memory updates ready to push.

**Earlier:** Tuesday 12 May 2026 ~22:00 demo-eve last-light close — **`v5.9.0.13.26` LIVE on irlid.co.uk; multi-fp `BOOTSTRAP_DEVELOPER_FP` shipped, 8 Pro now developer-tier on QR-login.** Worker `irlid-api-org/src/index.js` got two new helpers (`bootstrapDeveloperFps(env)` + `isBootstrapDeveloperFp(env, pub_fp)`) and all eight call sites refactored — six Form-A boolean checks collapsed to one-line helper calls, one Form-B debug-emitting site kept its diagnostic surface adapted for multi-fp (now reports count + total length). Secret updated via `"TvklFsivZk68R67j,65u-S-W_NFxr8u1L" | npx wrangler secret put BOOTSTRAP_DEVELOPER_FP` (desktop + 8 Pro fps). Wrangler-deployed, Captain QR-signed-in on 8 Pro, dashboard sidebar reads "Signed in as Developer (Super-Admin) (developer)" — confirms 8 Pro fp is being accepted. Phone-side doorman bind path **should** now work end-to-end but not yet verified — that's tomorrow morning's first test. Two Mr. Data briefs written at live repo root and queued via sequential prompt: `HANDOVER-BackgroundSymmetric.md` (`v5.9.0.13.27` — bg image symmetric duplicate via mirror-across-centre toggle, Captain wants for the dragon/skull bookend effect) + `HANDOVER-RoleLabelsCarryOver.md` (`v5.9.0.13.28` — role vocabulary labels carry to "Viewing as" dropdown + role-pill tooltips/initials, ~30 lines of diff). Both are clean-scoped, non-conflicting. Demo Wednesday 13 May is now phone-runnable in principle — multi-fp closes the auth asymmetry that was scenario B's deadweight in the earlier successor letter.

**Earlier:** Tuesday 12 May 2026 ~19:00 demo-eve final close — **`v5.9.0.13.25` LIVE on irlid.co.uk; fresh-attendee doorman bind proven end-to-end via desktop.** Earlier ~2am close had flagged the bind as silently failing — re-tested after Captain pulled an old orange-QR screenshot from his recycle bin and the **desktop path worked first try**. Poppy Austin (daughter, fresh device) bound at 18:43 via paste-orange-QR + Process scan + Bind in dashboard, scan_count=2. Re-tested after Captain deleted Kerry's binding: Kerry re-bound clean at 19:01, scan_count=2. **Two independent fresh-bind successes on v5.9.0.13.25.** The `.21`/`.25` Worker patches landed correctly; `wrangler tail` showed live traffic on every dashboard action. Phone-side bind still fails with "Staff HELLO proof cancelled" because Captain's 8 Pro pub_fp (`65u-S-W_NFxr8u1L`) doesn't match `BOOTSTRAP_DEVELOPER_FP` (`TvklFsivZk68R67j` — his desktop browser); on the phone he acts as non-developer and hits the Staff HELLO gate. **Demo plan:** doorman scans get processed from the desktop dashboard, not from staff phones. Multi-fp developer recognition is post-demo work. Captain noted demo may not happen at all (Donald uncertain, hardware uncertain — only guaranteed access is his phone + crappy tablet); even so, the proof-of-concept story is rock-solid.

**Earlier:** Tuesday 12 May 2026 demo-eve close — **`v5.9.0.13.20` LIVE on irlid.co.uk; demo-ready.** Smoke test run end-to-end through the demo flow on real hardware (Huawei tablet in audit mode + phone scanning + dashboard). All beats land: banner fires within 4s of each scan, celebration effects play smoothly, audit board updates live (Captain quote on watching Kerry's check-out propagate: *"very very nice :D ;D ;D"*), offline mode preserves optimistic rows with PENDING SYNC pill, reconnect catch-up fires celebrations for the surge of remote check-ins. Six post-Mr-Data-merge inline patches this afternoon (`v5.9.0.13.15` → `.20`): test-fire button + Role vocabulary panel + Website import placeholder (`.14`), Mr. Data's PR #105 Celebration text full controls + PR #106 per-effect intensity (`.15`/`.16` equivalent), prominent offline pill + SW cache bump (`.17`), z-index above fullscreen (`.18`), offline state INSIDE fullscreen + Refresh-while-offline toast (`.19`), pending_sync row preservation on offline poll (`.20`). Two new BOOTSTRAP §6 pitfalls captured. Memory + successor letter written. **Demo is on for Wednesday 13 May.**

**Earlier same day:** Tuesday 12 May 2026 late watch — **`v5.9.0.13.14` LIVE on irlid.co.uk.** Post-celebration-sprint tidying watch added: QR test tools got a "Test fire animation" button (fires celebration matching active mode without needing a second device); new "Role vocabulary" Settings panel with 6 presets (Default / Education / Corporate / Healthcare / Hospitality / Conference / Custom) + 5 role label inputs (Attendee/Staff/Manager/Lead admin/Developer) + Worker validator + lost-fields verifier + `roleLabel(roleKey)` helper on window for Phase 2 sweep; Website theme import placeholder card with disabled inputs (frame-ready for the v5.5.8 feature). Two Mr. Data briefs now queued at test env: `HANDOVER-CelebrationTextAndIntensity.md` (Text full controls + per-effect Muted/Vivid) and `HANDOVER-RoleVocabularySweep.md` (CSV Role column + dashboard render-site sweep to call `roleLabel()`). Number One signing off at ~97% usage.

**Earlier same day:** Tuesday 12 May 2026 mid-afternoon watch — **`v5.9.0.13.13` LIVE on irlid.co.uk.** Single long watch carved through the celebration animation panel from "checkbox-driven layered effect architecture" through to a working set of five orthogonal effects firing visibly. Headline: Celebration Animation panel now renders as a 6-checkbox round (Pulse / Background / QR / Glow / Pattern / Text) instead of a single-mode dropdown; each effect has its own toggle row of variant chips; layered effects compose on real check-ins via a per-row poll-detection path plus a centre-screen banner that fires on every check-in event (local OR remote-poll-detected). 13 inline patches over the day — `v5.9.0.13.1` preview-chip anchor + 9-cell position grid fix; `.2` Simple pulse mode + remote check-in poll for celebration; `.3` check-round architecture (5 effects + layered class building); `.4` opacity gate + body-flash + diagnostic logs; `.5` glow inset shadows + preview position fix; `.6` big centre-screen check-in event banner ("`<Name>` checked IN/OUT", fires on every event regardless of CSS animation visibility — Captain's "just print on screen first name surname checked in/out" directive); `.7` verbose poll diagnostics; `.8` boosted body-flash visibility; `.9` `<details>/<summary>` ripped out and replaced with `<div>` + `<label for>` because `summary.click + preventDefault` was silently suppressing checkbox toggle on the underlying input (Glow worked only because it was pre-checked); `.10` BG → page, QR → actual element, glow → palette cycle, pattern → visible (mix-blend-mode dropped); `.11` bg-overlay as DOM element + 4-step dissolve + Wobble motion; `.12` bg-overlay reliability + Rotate=360° spin + halo retired from Background Anim modes; `.13` palette wrap-around (root CSS defaults for cycle-1..7 were bleeding through short palettes — "red as default 2nd colour") + simple Text effect on/off checkbox. The toast banner is the dock-reach: even if every fancy effect were stripped tomorrow, the banner alone is a demo-worthy feature.

**Standby state at watch refresh:** Captain testing live; Mr. Data expected back in ~30 min (Codex credits reset ~10:49 BST). `HANDOVER-CelebrationTextAndIntensity.md` brief queued for him — full Text effect (template / position / size controls) + per-effect Muted/Vivid intensity toggles for Pulse/Background/QR/Pattern (Glow already has Muted/Vivid/Hyper). v5.9.0.14 brief (the older 10-halo-variants design) was retired in favour of this orthogonal architecture; Mr. Data was already correctly refusing to start it because precondition wasn't met. **Open issues for next watch:** sign-out paper-cut from 10 May still parked (two-clicks-first-time); celebration animations only visible on Check-in panel (small QR + fullscreen) — Dashboard view shows banner + body-flash but not QR-level effects because #venueQRWrap is `display:none` on that panel; demo flow for tomorrow assumes Captain stays on Check-in panel during the demo.

**Original 11 May morning watch entry below this kept for narrative continuity:**
...
| Last meaningful change | v5.9.0.13.20 (12 May demo-eve) — six post-Mr-Data-merge patches landed: QR test-fire button + Role vocabulary panel + Website import placeholder (.14); Mr. Data PR #105 Text full controls + PR #106 per-effect intensity (.15/.16); prominent offline pill + SW cache bump (.17); offline state above fullscreen QR (.18, .19); pending_sync row preservation on offline poll (.20). Smoke tested end-to-end on hardware. Demo-ready. |
...
| Celebration Text + per-effect intensity | Mr. Data (queued) | `v5.9.0.13.x` (next) | `IRLid-TestEnvironment\HANDOVER-CelebrationTextAndIntensity.md` | Drafted 12 May. Text effect = template/position/size controls (replaces simple on/off currently live). Per-effect Muted/Vivid toggle for Pulse/Background/QR/Pattern; Glow already has its 3-stop. Awaits Codex credits reset. |
