...
**Last refreshed:** Saturday 16 May 2026 morning — **`v5.10.2` LIVE on irlid.co.uk; `v5.10.1` Path B merged + `v5.10.2` Settings polish shipped clean in a single fuzzy-day watch; bootstrap-pointer propagation chain repaired retroactively + structurally.** Captain merged Mr. Data's Path B PR #25 at `7c7c146` after diff review + hardware smoke on 8 Pro + desktop; Path B is `requireSignedAction` accepting Bearer-resolved authority (signature for non-repudiation, authority from session token — closes the desktop-binding gap that had been blocking since the per-action WebAuthn architecture landed). Number One inline shipped `v5.10.2` Settings panel polish (`9f7c220`): `OrgCheckin.html` line 3710 placeholder was leaking `bunhead.github.io/IRLid-TestEnvironment/OrgCheckin.html` into live production UI; fixed to `example.com/staff-page` matching sibling field convention. Build pill `v5.10.1` → `v5.10.2`; SW cache `irlid-shell-v10` → `irlid-shell-v11`. Spencer hardware-verified live: scan_count=3 status IN, dashboard "Updated just now". **Structural inscription work (`2240368`):** the 15 May afternoon successor letter shipped without the BOOTSTRAP.md §10 bootstrap-pointer block — this watch's Number One arrived without knowing BOOTSTRAP.md existed, asked Captain where local repo path was documented (it was in BOOTSTRAP §3 in five places the whole time). Retroactive prepend of §10 pointer block on the 15 May letter + new `memory/letters/_TEMPLATE-successor-letter.md` scaffold so future watch-closes copy a structurally-correct artefact rather than freehanding the opening. **Versioning convention clarified:** 3-part `vX.Y.Z` for shipped work + single-letter suffixes (`a`/`b`/`c`) for tiny patches — drift toward 4-part dotted patches rolled back per BOOTSTRAP §4's documented rule. Queued briefs version-bumped accordingly: A1 → `v5.10.3`, A2 → `v5.10.4`.
...
| Last meaningful change | **`v5.10.2`** (16 May morning, commit `9f7c220`) — Settings panel Staff post-accept redirect URL placeholder cleanup (was leaking `bunhead.github.io/IRLid-TestEnvironment/OrgCheckin.html` test-env URL into live production Settings UI; fixed to `example.com/staff-page` matching sibling field convention). SW cache `irlid-shell-v10` → `v11`. Built on `v5.10.1` (16 May, `7c7c146`) Path B merge — `requireSignedAction` accepts Bearer-resolved developer authority, signature now binds the actor for non-repudiation while authority resolves through the session token. Closes the per-device bootstrap-fp dance; desktop binding now works. Live, hardware-verified Spencer scan_count=3. |
