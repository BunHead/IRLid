...
**Last refreshed:** Tuesday 12 May 2026 demo-eve close — **`v5.9.0.13.20` LIVE on irlid.co.uk; demo-ready.** Smoke test run end-to-end through the demo flow on real hardware (Huawei tablet in audit mode + phone scanning + dashboard). All beats land: banner fires within 4s of each scan, celebration effects play smoothly, audit board updates live (Captain quote on watching Kerry's check-out propagate: *"very very nice :D ;D ;D"*), offline mode preserves optimistic rows with PENDING SYNC pill, reconnect catch-up fires celebrations for the surge of remote check-ins. Six post-Mr-Data-merge inline patches this afternoon (`v5.9.0.13.15` → `.20`): test-fire button + Role vocabulary panel + Website import placeholder (`.14`), Mr. Data's PR #105 Celebration text full controls + PR #106 per-effect intensity (`.15`/`.16` equivalent), prominent offline pill + SW cache bump (`.17`), z-index above fullscreen (`.18`), offline state INSIDE fullscreen + Refresh-while-offline toast (`.19`), pending_sync row preservation on offline poll (`.20`). Two new BOOTSTRAP §6 pitfalls captured. Memory + successor letter written. **Demo is on for Wednesday 13 May.**
