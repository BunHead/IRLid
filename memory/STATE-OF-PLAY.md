...
| Last meaningful change | `v5.9.0.13.34` (13 May evening, PR #20) — production lockdown hiding "Viewing as" prototype-role toolbar for non-developers. Single CSS gate, +11/-1 in `OrgCheckin.html`. |
...
| Captain's Developer membership | `org_memberships` row inserted 10 May evening — role `developer` on Test Event, `pub_fp` `TvklFsivZk68R67j` (desktop) + `65u-S-W_NFxr8u1L` (8 Pro) via multi-fp `BOOTSTRAP_DEVELOPER_FP` |
