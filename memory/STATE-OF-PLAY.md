...
**Last refreshed:** Tuesday 12 May 2026 ~2am demo-eve close — **`v5.9.0.13.25` LIVE on irlid.co.uk.** Late session pushed five more inline patches (`.21` → `.25`) chasing the doorman-bind path for fresh attendees. `.21` Worker accepts org_ Bearer as developer. `.22` action label styling on escalation rows. `.23` stripped orange QR (sig/hash/nonce removed, Level H → M). `.24` frontend verifier accepts stripped envelope. `.25` Worker `requireDevOrStaffSession` aligned to use shared `bootstrapDeveloperFromBearer` helper. **Open bug: clicking "Choose from list" → Poppy Austin → confirm OK does nothing visible — bind silently fails.** Full diagnostic dossier in `memory/open-bug-doorman-bind-2026-05-12.md` — start there next watch, specifically Step 1 (`wrangler tail` to confirm Worker `.25` actually deployed). Demo is Wednesday 13 May. **Proven demo path:** pre-add attendees to Expected list, they scan venue QR → green-flow celebrations fire. **Brittle demo path:** fresh-attendee orange-QR-then-bind. Workaround in dossier — Option C — demo with pre-added attendees only, skip the fresh-scan beat.
