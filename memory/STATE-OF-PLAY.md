...
| Last meaningful change | v5.9.0.7 (10 May evening watch 2 close) — full check-in + check-out cycle proven end-to-end on live with multi-round verification (3+ scans per attendee, 7 check-outs across session, hard-refresh durability confirmed). Four patches landed: v5.9.0.4 (bootstrap fp recovery), v5.9.0.5 (mock_org self-heal + hostname-gating), v5.9.0.6 (scan.html dynamic dashboardOrigin), v5.9.0.7 (SCAN_PAGE_URL + getQrScanDomain fallback dynamic). |
