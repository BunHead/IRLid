...
- **Number-One-authored brief → default ⚠️ REVIEW, never ✅ ACCEPT (4 June 2026).** When Number One *wrote* the HANDOVER brief for a Mr. Data PR, Number One is reviewing his own design — grading his own homework — so the independent check is structurally weak. In that case the verdict marker must default to **⚠️ REVIEW** (surface the design assumptions + what the hardware smoke must prove), not ✅ ACCEPT, even when the diff matches the brief perfectly. Reserve ✅ ACCEPT for PRs against a brief Number One did NOT author, or after a genuinely independent check (e.g. hand the diff to Reg/Barclay to red-team). Captain's catch — he merged a Number-One-authored expected_ids PR on an ACCEPT that should have been REVIEW; the code was safe, but the principle holds. The marker advertises *independence of judgement*, and self-review has none.
...
- **`git stash` across a revert boundary can re-introduce the reverted state.** Earned 29 May 2026 (`v5.11.25` revert). The intent was to revert a merged-then-failed PR while an unrelated mockup edit sat uncommitted in the working tree. The sequence used was: `git stash` (to clear the dirty tree so `git revert` would run) → `git revert HEAD -m 1 --no-edit` → `git stash pop`. The problem: the stash and the revert touched OVERLAPPING files. When the stash popped, it re-applied the stashed versions of those files — which still carried the v5.11.25 changes (the working tree had been mid-edit on top of the merged code). Net effect: the revert commit landed, but the pop immediately re-introduced `Build v5.11.25` strings into `Org.html` + `sw.js`, so the live site STILL showed the reverted version after deploy. Captain caught it on hardware (build pill still read `v5.11.25`); a grep confirmed the strings were back on disk. **Recovery:** finish the revert deterministically with `git checkout <pre-merge-sha> -- <each-file>` to pin the exact known-good content, then commit + push. **Discipline:** never stash across a revert boundary. If the tree is dirty when you need to revert, COMMIT the unrelated work first (even to a scratch branch), then revert cleanly, then cherry-pick the scratch work back. Committing before reverting keeps the two change-sets from ever overlapping in a stash. If you must use stash, verify post-pop with `git diff <pre-merge-sha> -- <reverted-files>` and a grep for the version string you meant to remove — don't trust the revert commit alone.
- **Wrangler / Cloudflare API timeout — retry once immediately before reaching for the dashboard fallback.** Earned 29 May 2026 (`v5.11.25` Worker deploy + the revert redeploy). `wrangler deploy` timed out once mid-watch, then succeeded on an immediate re-run with no other change — consistent with home-WAN flakiness (§6 timeout entry above) rather than a Cloudflare-side or auth problem. **Discipline:** on the FIRST wrangler timeout, just run the exact same command again straight away; it usually goes through. Only escalate to the Cloudflare dashboard Quick Edit fallback (documented 17 May) on a SECOND consecutive timeout. Saves the heavier copy-paste-into-dashboard dance for the cases that actually need it. (Pairs with the existing timeout entry — that one is the fallback; this one is the cheap first move.)
- **Windows PowerShell 5.1 lacks `[RandomNumberGenerator]::Fill()` — a failed RNG call can silently produce an ALL-ZEROS secret.** Earned 11 June 2026 (api_key rotation during the QR diet). The one-liner `$b = New-Object byte[] 32 ; [Security.Cryptography.RandomNumberGenerator]::Fill($b) ; $key = "org_" + ([Convert]::ToBase64String($b)...)` threw `MethodNotFound` on the `Fill` call (it's .NET Core / PowerShell 7+ only) — but with `;` separators the line CARRIED ON, and `$b` stayed as `New-Object` initialised it: 32 zero bytes. The wrangler D1 UPDATE then executed successfully, setting the org's api_key to `org_AAAAAAAA...` — base64 of all zeros, the most guessable key in existence. Same genus as the 0x16 Ctrl+V trap: the secret-setting machinery succeeds while the secret itself is garbage. **Discipline:** (1) on Windows PowerShell 5.1 use `[Security.Cryptography.RandomNumberGenerator]::Create().GetBytes($b)` — instance method, works everywhere; (2) ALWAYS echo a verifiable fragment of the generated secret (e.g. `$key.Substring(0,12)`) in the same one-liner so a row of A's is caught by eyeball before anything trusts it; (3) treat any red error text ABOVE a green "Executed 1 command" as a failed pipeline, not a cosmetic warning — `;` does not short-circuit on error the way `&&` does.
- **`org_receipts.checkin_id` carries an FK to `org_checkins(id)` — any DELETE of check-in rows must clear the receipts FIRST.** Earned 11 June 2026 (Captain's Delete-record on Becky aborted with `D1_ERROR: FOREIGN KEY constraint failed: SQLITE_CONSTRAINT_FOREIGNKEY`). The v6.0 receipt bridge quietly made receipts load-bearing: D1 refuses to forget a check-in while a receipt row still references it. Both deletion paths were patched in `v6.4.4b` (`orgExpectedDeleteFull` cascade + the clear-attendance debug path): `DELETE FROM org_receipts WHERE org_id=? AND checkin_id IN (SELECT id FROM org_checkins WHERE ...)` before the check-ins themselves. **Discipline going forward:** whenever a NEW table gains a `REFERENCES org_checkins(id)` (or any FK into a table with delete paths), grep for every `DELETE FROM <referenced-table>` in the Worker and teach each one about the new tenant in the same PR that adds the FK. The FK doing its job looks exactly like a broken delete button to the user. Design note: Delete-record remains the deliberate erasure path — the org's stored receipt copies go, but the attendee's downloaded receipts remain theirs and still verify (the broker-not-store philosophy holding under deletion).
- **A hardware failure on an UNVERIFIED deploy is a deploy problem until the build pill proves otherwise.** Earned 30 May 2026 (the v5.11.25c morning). Number One shipped an inline Org.html fix, gave Captain a `git add Org.html sw.js ; commit ; push`, and moved on. Captain ran an empty `git commit --allow-empty -m 'redeploy'` but the content-commit command never actually ran — so `Org.html`/`sw.js` sat MODIFIED in the working tree, never on origin, never on any device. Three rounds of hardware testing "failed" with the device on the OLD version the whole time. We spent real effort theorising about runtime causes (scanner stripping fragments, `/user/orgs` not returning the org, SW cache) when the truth was simply: **the fix was never on the phone.** The `git status --short` during squaring-away exposed it (` M Org.html`, ` M sw.js`); once pushed for real (`9db85e3`) the loop closed first try. **Discipline:** before concluding an inline fix "doesn't work" from a hardware test, verify TWO things — (1) `git status --short` is CLEAN for the patched files (they're actually committed), and `git log --oneline -1` shows your content commit on origin; (2) the build pill ON THE DEVICE reads the new version. If either check fails, it's a deploy/cache problem, not a logic problem — fix that first. Corollary: when you hand Captain a multi-command one-liner, the content commit and the push are the load-bearing parts; an empty redeploy commit is NOT a substitute and will mask a missing content commit (the Pages deploy goes green serving the OLD code). Watch the `git push` output for the actual file list, not just "main -> main".
